...
The Census updater accepts only HTTPS URLs from approved ABS hosts. It changes source content only when validated official figures differ from the current values. The scheduled GitHub workflow opens a pull request instead of pushing directly to `master`.
...
Page CSS bundles are generated during the build from the modular files in `src/assets/css/`. Do not add or edit `src/assets/css/bundles`; that directory is intentionally absent from source control.
