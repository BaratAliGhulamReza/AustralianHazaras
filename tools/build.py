...
def build_css_bundles() -> str:
    ...
    for path in sorted(output_root.glob('*.css')):
...
def build():
    ...
    asset_version = build_css_bundles()
    ...
    for name,(paths,title,desc) in specs.items():
        ...
        html=render(template,{'HEADER':header,'FOOTER':footer,'PAGE_DATA':compact(pdata),'STRUCTURED_DATA':structured(title,desc,base+name),'ASSET_VERSION':asset_version})
